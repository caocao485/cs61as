...
(define (get-space-nums prev)
  (let ((char (peek-char)))
    (if(not(eq? char #\space))
       prev
       (begin
         (read-char)
         (get-space-nums (+ prev 1))))))
...
(define (py-read)
  (define (get-indent-and-tokens)
    ...
    (let ((space-nums (get-space-nums 0))) ;;强制执行,早于get-tokens
         (cons space-nums (get-tokens '()))))
  ...
  (define (get-tokens braces)
    ...
    (let ((char (peek-char)))
      (cond
       ((char-newline? char)
	(if (not (null? braces)) ;;如果有未闭合的括号则继续读取
	...
	(if (not (null? braces))
       ...
       ((memq char (list #\[ #\( #\{))
        (let ((s (char->symbol (read-char))))
          (cons s (get-tokens (cons char braces)))))  ;;这里会包裹括号到braces中
       ((memq char (list #\] #\) #\}))
	(if (and (not (null? braces)) (eq? char (reverse-brace (car braces))))
            (let ((t (char->symbol (read-char))))
              (cons t (get-tokens (cdr braces)))) ;;闭合则去掉括号
	...
	(let ((token (get-token (char->symbol (read-char)))))
	  (cond
           ((and (string? token)
                 ...
                 (char-numeric? (string-ref token 1)));;小数点
            (begin (display token)
  ...
  (define (get-num num-so-far)
    ...
    (let ((has-dot false))
      (define (iter number)
        (let ((char (peek-char)))
          (cond((eof-object? char)(read-char)number)
               ((char-numeric? char)
                (iter(word number(char->symbol(read-char)))))
               ((and(eq? char #\.)
                    (not has-dot))
                (set! has-dot true)
                (iter(word number(char->symbol(read-char)))))
    ...
    (define (iter lst)
      (let ((char (peek-char)))
        (cond((eof-object? char)
              (read-error "SyntaxError: End of file inside String"))
             ((memq char (list #\" #\'))
              (if (eq? char type)
                  (begin
                    (read-char)
                    (append lst))
                  (iter (append lst (list (read-char))))))
             (else
              (iter (append lst (list (read-char))))))))
    (iter '()))
