		 ...
		 (handle-infix (py-apply (ask val (lookup-op token))
	      ...
	      ((and? token)
	       (if (not (ask val 'true?))
                   (begin
                     (eat-tokens line-obj)
                     (handle-infix *PY-FALSE* line-obj env))
                   (let ((rhs (eval-item line-obj env)))
                     (if (not (ask rhs 'true?))
                         (handle-infix *PY-FALSE*
                                       line-obj
                                       env)
                         (handle-infix rhs
                                       line-obj
	      ...
	      ((in? token)
	       (let ((rhs (eval-item line-obj env)))
                 (handle-infix (ask rhs '__contains__ val)
                               line-obj
                               env)))
	      ((not? token)
	       (let ((next-token (ask line-obj 'next)))
                 (if(not(in? next-token))
                    (py-error  "SyntaxError: not found token: in")
                    (let ((rhs (eval-item line-obj env)))
                      (handle-infix (negate-bool
                                     (ask rhs '__contains__ val))
                                    line-obj
                                    env)))))
                 
	      ;; dot syntax message: val.msg
        ((dotted? token)
          (let ((func (ask val (remove-dot token))))      ;gets the py-function ;;这里导致__contains__出错，修改需要修改__contains__方法返回的值的数据结构
            (if (and (not (ask line-obj 'empty?))
	  ...
	  (let ((line (py-read)))
	    (define (helper)
	      ...
	      (cond ((null? (tokens line)) (set! unindented-line #f) '())
		    ((> (indentation line) new-indent) ;;普通行，多一个缩进也会导致错误
		     (py-error "SyntaxError: Unexpected indent"))
		    ((< (indentation line) new-indent) ;;相比减少缩进，要么跳出循环/定义过程，要么是else、elif部分
		     (if (and (= (indentation line) old-indent)
			      (not (null? (tokens line)))
			      (memq (car (tokens line)) '(elif else))) ;;else elif部分block
			 (let ((trailing-block (make-block (make-line-obj line)
							   env)))
			   (if (not unindented-line) 
			       (list trailing-block)
			       (begin (set! line unindented-line) ;;处理上一个未被处理的减少缩进的行
				      (set! unindented-line #f)
				      (cons trailing-block (helper))))) ;;下次helper循环处理line
			 (begin (set! unindented-line line) ;;未处理的行
				'())))
...
;;get-pred
(define (collect-block line-obj)
  (let ((token (ask line-obj 'next)))
      (if (colon? token)
          '()
          (cons token (collect-block line-obj)))))
(define (get-pred line-obj)
  (let ((indentation (ask line-obj 'indentation))
    (pred-list (collect-block line-obj)))
    (cons indentation pred-list)))

;; Conditionals
(define (make-if-block line-obj env)
  (let ((pred (get-pred line-obj))
        (body (read-block (ask line-obj 'indentation) env)))
	  (list '*BLOCK* '*IF-BLOCK* pred body)))
(define (if-block-pred block)
  (caddr block))
(define (if-block-body block)
  (let ((body (cadddr block)))
...
(define (make-else-block line-obj env)
  (if (not (and (not (ask line-obj 'empty?))
		(colon? (ask line-obj 'next))
		(ask line-obj 'empty?)))
      (py-error "SyntaxError: invalid syntax" "hello world" (ask line-obj 'tokens))
...
(define (make-elif-block line-obj env)
  (let ((pred (get-pred line-obj))
        (body (read-block (ask line-obj 'indentation) env)))
	  (list '*BLOCK* '*ELIF-BLOCK* pred body)))
...
(define (eval-elif-block block env)
  (let ((pred (elif-block-pred block))
	(body (elif-block-body block))
	(else-clause (elif-block-else block)))
	(let ((bool-value (py-eval (make-line-obj pred) env)))
	  (if (ask bool-value 'true?)
              (eval-sequence body env)
	      (if else-clause
                  (let ((else-block (car(tokens else-clause))))
                    (if (else-block? else-block)
                      (eval-else-block else-block env)
                      (eval-item
                       (make-line-obj
                              (list '*DUMMY-INDENT*
                                    (list '*BLOCK* '*IF-BLOCK*
                                          (elif-block-pred else-block)
                                          (elif-block-body else-block))))
                                    env)))
		  *NONE*)))))
...
(define (make-for-block line-obj env)
  (let ((block-var (ask line-obj 'next)))
    (if (not (in? (ask line-obj 'next)))
        (py-error "SyntaxError: Expected "in", unencountered ")
        (let ((block-collection (collect-block line-obj))
             (body (read-block (ask line-obj 'indentation) env)))
          (list '*BLOCK* '*FOR-BLOCK* block-var block-collection body)))))
...
(define (eval-for-block block env)
  (let ((block-var-name (for-block-var block))
	(block-collection (for-block-collection block))
	(block-body (for-block-body block))
        (block-else (for-block-else block)))
    (let ((collection (py-eval (make-line-obj
                                (cons '*DUMMY-INDENT* block-collection)) env)))
      (let ((result (ask collection '__iter__  block-var-name block-body env)))
        (cond ((eq? result '*BREAK*)  *NONE*)
              (else
               (if block-else
                   (eval-item (make-line-obj block-else) env)
                   *NONE*)))))))
