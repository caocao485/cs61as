...
(define (negate-bool bool) (if (ask bool 'val) *PY-FALSE*  *PY-TRUE*))

(define (make-py-list val)
  (instantiate py-list val))

(define (list-contains-element? lst item comp-proc)
  (cond((null? lst)#f)
       ((comp-proc item (car lst))#t)
       (else(list-contains-element? (cdr lst) item comp-proc)))) 
(define-class (py-list val)
  ...
  (method (__contains__ other)
    (let ((v (list-contains-element? val other comp-proc)))
      (if v
        (make-py-bool #t)
        (make-py-bool #f))))
  (method (append)
...
(define-class (math)
  ...
  (method (sin)
    (make-py-primitive
     'sin
     (lambda (num) (make-py0-num (sin (ask num 'val))))))
  (method (tan)
    (make-py-primitive
     'tan
