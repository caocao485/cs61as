...
(define (max-value pair-list)
  (define (iter mvalue rlst)
    (if(null? rlst)
       mvalue
       (iter ((lambda(prev current)
                (if(< (cdr prev)
                      (cdr current))
                   current
                   prev))
              ...
              (car rlst)) 
             (cdr rlst))))
  (iter (car pair-list) (cdr pair-list)))

(define (real-most-frequent data)
  (max-value (mapreduce mapper reducer 0 data))
  )
